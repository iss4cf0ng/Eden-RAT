...
HTTP_SERVER_IP = '[SERVER_IP]'
HTTP_SERVER_PORT = int('[SERVER_PORT]')
...
class Client:
    ...
    def sendcipher(self, nCmd: int, nParam: int, szMsg: str):
        ...
        self.http_send(nCmd=nCmd, nParam=nParam, szData=szEncMsg)
    ...
    def get_body_post(self, szData: str, szResource = '/') -> str:
        return self.get_req_body(szData, 'POST', szResource)

    def get_body_get(self, szData: str, szResource = '/') -> str:
        return self.get_req_body(szData, 'GET', szResource)

    def get_req_body(self, szData: str, szMethod = 'POST', szResource = '/') -> str:
    ...
    def http_send(self, nCmd: int, nParam: int, szData: str = ''):
        buffer = C2P(nCmd=nCmd, nParam=nParam, abData=szData.encode('ascii')).get_buffer()
        b64 = base64.b64encode(buffer).decode('ascii')
        req = self.get_body_post(szData=b64, szResource='/')
...
def handler(serv_sock: socket.socket):
    ...
    while serv_sock.fileno() != -1:
        resp = serv_sock.recv(C2P.BUFFER_MAX_LENGTH)
        if not resp:
            continue

        szResp = resp.decode('utf-8')

        headers, body = szResp.split('\r\n\r\n', 1)
        ...
        szb64msg = abBody.decode('utf-8')
        abMsg = base64.b64decode(szb64msg)

        c2p = C2P(abBuffer=abMsg)
        tp = c2p.get_header()
        nCmd = tp[0]
        nParam = tp[1]
        nLength = tp[2]
        ...
        if nCmd == 0:
            ...
            if nParam == 1:
                ...
                clnt.http_send(1, 2, Encoder.stre2b64(szMsg))
                ...
                clnt.http_send(1, 4, sz_b64_resp)

        elif nCmd == 2:
            if nParam == 0:
                clnt.sendcipher(2, 1, g_szTag)
            elif nParam == 2:
                plain = clnt.pAES.decrypt_cbc(abMsg)
