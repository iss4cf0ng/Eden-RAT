...
class EZRSA:
    def __init__(self, nKeyBits: int):
        self.nKeyBits = nKeyBits
        
    def b64encode(self, num):
        return base64.b64encode(num.to_bytes((num.bit_length() + 7) // 8, 'big')).decode('utf-8')
    ...
    def encode_public_key(self, n, e):
        root = ET.Element("RSAKeyValue")
        ET.SubElement(root, "Modulus").text = self.b64encode(n)
        ET.SubElement(root, "Exponent").text = self.b64encode(e)
        return ET.tostring(root, encoding='utf-8').decode('utf-8')

    def encode_private_key(self, n, e, d, p, q):
        root = ET.Element("RSAKeyValue")
        ET.SubElement(root, "Modulus").text = self.b64encode(n)
        ET.SubElement(root, "Exponent").text = self.b64encode(e)
        ET.SubElement(root, "D").text = self.b64encode(d)
        ET.SubElement(root, "P").text = self.b64encode(p)
        ET.SubElement(root, "Q").text = self.b64encode(q)
        return ET.tostring(root, encoding='utf-8').decode('utf-8')

    def encode_to_xml(self, n, e, d, p, q):
        root = ET.Element("RSAKeyValue")
        ET.SubElement(root, "Modulus").text = self.b64encode(n)
        ET.SubElement(root, "Exponent").text = self.b64encode(e)
        ET.SubElement(root, "D").text = self.b64encode(d)
...
def main():
    ...
    pubKey = PRSA.xml_to_rsa_key(xml_PubKey, False)
    privKey = PRSA.xml_to_rsa_key(xml_privKey, True)
